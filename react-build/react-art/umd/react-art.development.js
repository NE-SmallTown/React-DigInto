...
function updateEventComponent$1(current, workInProgress, renderExpirationTime) {
  var nextProps = workInProgress.pendingProps;
  var nextChildren = nextProps.children;
...
function bailoutOnAlreadyFinishedWork(current, workInProgress, renderExpirationTime) {
  ...
  if (childExpirationTime < renderExpirationTime) {
    ...
    cloneChildFibers(current, workInProgress); // 把 current 的 child 复制给 workInProgress.child
    return workInProgress.child;
  }
}
...
function beginWork$1(current, workInProgress, renderExpirationTime) {
  ...
  switch (workInProgress.tag) {
    ...
    case EventComponent:
      {
        if (enableEventAPI) {
          return updateEventComponent$1(current, workInProgress, renderExpirationTime);
        }
        break;
      }
    case EventTarget:
...
function commitUnmount(current) {
  ...
  switch (current.tag) {
    ...
    case EventComponent:
      {
        if (enableEventAPI) {
          var eventComponentInstance = current.stateNode;
