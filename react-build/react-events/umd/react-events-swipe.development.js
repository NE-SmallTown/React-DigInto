...
(function (global, factory) {
	...
	(global.ReactEventsSwipe = factory());
...
var rootEventTypes = ['pointerup', 'pointercancel', { name: 'pointermove', passive: false }];
...
function createSwipeEvent(type, target, eventData) {
  return objectAssign({
    target: target,
    type: type
  }, eventData);
...
function dispatchSwipeEvent(context, name, listener, state, discrete, eventData) {
  ...
  var syntheticEvent = createSwipeEvent(name, target, eventData);
  context.dispatchEvent(syntheticEvent, listener, { discrete: discrete });
}
