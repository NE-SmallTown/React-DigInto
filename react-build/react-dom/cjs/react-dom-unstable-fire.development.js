...
var eventResponderContext = {
  ...
  addRootEventTypes: function (doc, rootEventTypes) {
    ...
    for (var i = 0; i < rootEventTypes.length; i++) {
      ...
      if (rootEventComponentInstances === undefined) {
        ...
        rootEventTypesToEventComponentInstances.set(topLevelEventType, rootEventComponentInstances);
      }
      var componentInstance = currentInstance;
      var rootEventTypesSet = componentInstance.rootEventTypes;
      if (rootEventTypesSet === null) {
        rootEventTypesSet = componentInstance.rootEventTypes = new Set();
      }
      (function () {
        if (!!rootEventTypesSet.has(topLevelEventType)) {
          throw ReactError('addRootEventTypes() found a duplicate root event type of "' + rootEventType + '". This might be because the event type exists in the event responder "rootEventTypes" array or because of a previous addRootEventTypes() using this root event type.');
        }
      })();
  ...
  removeRootEventTypes: function (rootEventTypes) {
    ...
    for (var i = 0; i < rootEventTypes.length; i++) {
      var rootEventType = rootEventTypes[i];
      var topLevelEventType = typeof rootEventType === 'string' ? rootEventType : rootEventType.name;
      var rootEventComponents = rootEventTypesToEventComponentInstances.get(topLevelEventType);
  ...
  getEventTargetsFromTarget: function (target, queryType, queryKey) {
    validateResponderContext();
    var eventTargetHostComponents = [];
    var node = getClosestInstanceFromNode(target);
    ...
    while (node !== null) {
      ...
      while (child !== null) {
        if (child.tag === EventTarget && queryEventTarget(child, queryType, queryKey)) {
          var _props = child.stateNode.props;
          var parent = child.return;
...
function queryEventTarget(child, queryType, queryKey) {
...
function getTargetEventResponderInstances(topLevelType, targetFiber) {
...
function getRootEventResponderInstances(topLevelType) {
  var eventResponderInstances = [];
  var rootEventInstances = rootEventTypesToEventComponentInstances.get(topLevelType);
  if (rootEventInstances !== undefined) {
    ...
    for (var i = 0; i < rootEventComponentInstances.length; i++) {
      ...
      if (currentOwner === null || currentOwner === rootEventComponentInstance) {
...
function traverseAndHandleEventResponderInstances(topLevelType, targetFiber, nativeEvent, nativeEventTarget, eventSystemFlags) {
  ...
  var targetEventResponderInstances = getTargetEventResponderInstances(topLevelType, targetFiber);
  ...
  if (length > 0) {
    ...
    for (i = length; i-- > 0;) {
      ...
      var responder = targetEventResponderInstance.responder,
          _props2 = targetEventResponderInstance.props,
      ...
      if (eventListener !== undefined) {
        ...
        eventListener(responderEvent, eventResponderContext, _props2, state);
      }
    }
    ...
    for (i = 0; i < length; i++) {
      ...
      var responder = _targetEventResponderInstance.responder,
          _props3 = _targetEventResponderInstance.props,
      ...
      if (_eventListener !== undefined) {
        ...
        _eventListener(responderEvent, eventResponderContext, _props3, state);
      }
    }
  }
  // Root phase
  var rootEventResponderInstances = getRootEventResponderInstances(topLevelType);
  ...
  if (length > 0) {
    for (i = 0; i < length; i++) {
      ...
      var responder = rootEventResponderInstance.responder,
          _props4 = rootEventResponderInstance.props,
      ...
      if (_eventListener2 !== undefined) {
        ...
        _eventListener2(responderEvent, eventResponderContext, _props4, state);
      }
    }
  }
}
  ...
  try {
    for (var i = 0; i < listeningInstances.length; i++) {
      ...
      if (onOwnershipChange !== undefined) {
        onOwnershipChange(eventResponderContext, _props5, state);
      }
    }
  } finally {
...
function mountEventResponder(eventComponentInstance) {
  ...
  if (onMount !== undefined) {
    var _props6 = eventComponentInstance.props,
        state = eventComponentInstance.state;

    currentEventQueue = createEventQueue();
    currentInstance = eventComponentInstance;
    try {
      onMount(eventResponderContext, _props6, state);
    } finally {
      currentEventQueue = null;
      currentInstance = null;
      currentTimers = null;
...
function unmountEventResponder(eventComponentInstance) {
  ...
  if (onUnmount !== undefined) {
    var _props7 = eventComponentInstance.props,
        state = eventComponentInstance.state;

    currentEventQueue = createEventQueue();
    currentInstance = eventComponentInstance;
    try {
      onUnmount(eventResponderContext, _props7, state);
    } finally {
      currentEventQueue = null;
      currentInstance = null;
      currentTimers = null;
...
function processUpdateQueue(workInProgress, queue, props, instance, renderExpirationTime) {
  ...
  // processUpdateQueue 就是改变 baseState，firstUpdate 以及 expriationTime
  ...
  while (update !== null) {
    ...
    if (_updateExpirationTime < renderExpirationTime) {
      ...
      // Since this update will remain in the list, update the remaining
      // expiration time.
      // 如果 WipFiber 的 expriationTime 比 update 的还要远，那么提升 WipFiber 的 expriationTime
      ...
      // a new result.
      // updateExpirationTime <= renderExpirationTime 说明 WipFiber 需要在当前帧更新，所以要先计算 state
      resultState = getStateFromUpdate(workInProgress, queue, update, resultState, props, instance);
  ...
  // 所有相关的 update 都处理完毕，便可以设置 baseState 为新的 state
...
function updateClassInstance(current, workInProgress, ctor, newProps, renderExpirationTime) {
  ...
  // 不懂。instance.state 不等于 oldState 是因为可能有 ErrorBoundary 的缘故？
  var oldState = workInProgress.memoizedState;
  var newState = instance.state = oldState;
  var updateQueue = workInProgress.updateQueue;
  if (updateQueue !== null) {
...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function reconcileChildrenArray(returnFiber, currentFirstChild, newChildren, expirationTime) {
    ...
    var oldFiber = currentFirstChild; // 旧的节点，一开始是 null
    ...
    for (; oldFiber !== null && newIdx < newChildren.length; newIdx++) {
      ...
      var newFiber = updateSlot(returnFiber, oldFiber, newChildren[newIdx], // 新的节点
      expirationTime);
      if (newFiber === null) {
  ...
  function reconcileSingleElement(returnFiber, currentFirstChild, element, expirationTime) {
    ...
    while (child !== null) {
      ...
      if (child.key === key) {
        if (child.tag === Fragment ? element.type === REACT_FRAGMENT_TYPE : child.elementType === element.type) {
          // 为什么明明都是 singleElement 了，child.sibling 还可能是 null 呢？
          ...
          //   );
          // }
          // 那是因为比如像上面这样，Stateful 这个组件由于 key 一样被复用了，所以当 condition 由 false 变为 true 之后
          // child 还是 condition 为 false 的那个 Stateful 的实例，这个实例是有 sibling 的（即 div）
          // if (child.sibling !== null) {
          //   throw Error(777777777777)
          // }
    ...
    if (element.type === REACT_FRAGMENT_TYPE) {
      ...
      // 子 fiber 集成父 fiber 的 mode，并且和父 fiber 永远相同的 expriationTime
      returnFiber.mode, expirationTime);
...
function updateHostRoot(current$$1, workInProgress, renderExpirationTime) {
  ...
  // nextState 就是处理完更新队列后更新后的 state
  var nextState = workInProgress.memoizedState;
  // Caution: React DevTools currently depends on this property
  // being called "element".
  var nextChildren = nextState.element; // state.element 实际上就是 root，即 ReactDOM.render(<App />, ...) 里的 <App />
  nextChildren === prevChildren && console.log(132131312);
...
function bailoutOnAlreadyFinishedWork(current$$1, workInProgress, renderExpirationTime) {
  ...
  if (childExpirationTime < renderExpirationTime) {
    ...
    // fibers and continue.
    // 当前 fiber 没有任务需要执行，但是它的子节点有任务需要执行
    // 那么 clone child 并返回，sheduler 会对子节点再次进行处理（workloop）
    cloneChildFibers(current$$1, workInProgress); // 把 current 的 child 复制给 workInProgress.child
    return workInProgress.child;
  }
}
...
function beginWork$1(current$$1, workInProgress, renderExpirationTime) {
  ...
  if (current$$1 !== null) {
    ...
    if (oldProps !== newProps || hasContextChanged()) {
      ...
      // 如果没有任务，或者过期时间是后面的帧（updateExpirationTime > renderExpirationTime），则跳过
      didReceiveUpdate = true;
    } else if (updateExpirationTime < renderExpirationTime) {
      didReceiveUpdate = false;
      // https://blog.csdn.net/ly402609921/article/details/50419083
      // Bailout 就是指没办法优化了
      // 这里即指在不进入 begin 阶段的情况下，对于没有任务需要执行的 fiber 的而已，只能 Bailout
      // 在 React 里面，Bailout 就是指不需要更新这个 fiber，即等价于 sCU 返回 false。
      // 这里即退出当前的 fiber，转而去处理当前 fiber 的 child
    ...
    case HostText:
      {
        ...
        if (current && workInProgress.stateNode != null) {
          ...
          if (_wasHydrated) {
            ...
            // 将创建的真实的 DOM 节点赋值给 fiber 的 stateNode
...
// workLoop 就是和浏览器的 event loop 一样的，不断执行每一个小的任务单元，直到没有任务（即直至执行到某个 fiber 的 child 没有任务为止）
function workLoop() {
...
function performUnitOfWork(unitOfWork) {
  ...
  if (enableProfilerTimer && (unitOfWork.mode & ProfileMode) !== NoContext) {
    ...
    // 不懂。有点意思，意思是工作永远是在 wipFiber 上工作？
    // 所以即便现在是在处理 workInProgress，传给 beginWork 的也是 workInProgress.alternate？
    next = beginWork$$1(current$$1, unitOfWork, renderExpirationTime);
  }

  resetCurrentFiber();
  unitOfWork.memoizedProps = unitOfWork.pendingProps;
  // next 为空表示 next fiber 的 child 为空了
  // 即 next 这个 fiber 没有子节点（比如一个 string，number，或者 JSX 里面本身就是 null）
  // 那么这种情况不可能再产生新的工作（因为没有子 fiber），所以直接 complete
  if (next === null) {
...
function completeUnitOfWork(unitOfWork) {
  ...
  do {
    ...
    if (siblingFiber !== null) {
      ...
      // 如果还有 sibling 需要处理，则 return 这个 sibling，然后交给 performWork 去处理
      // 因为本函数（completeWork）只负责 complete 而不负责 perform
      return siblingFiber;
    }

    // Otherwise, return to the parent
    // 如果 siblingFiber === null 了说明 wipFiber 的 children 全部都已经 complete 了
    // 所以接下来应该 complete wipFiber 本身了，因为本函数是 complete，所以只要是 complete
    // 相关的就应该一直处理下去，否则就应该 return（比如上面的 return）
...
function updateContainerAtExpirationTime(element, // 实际上就是 root，即 ReactDOM.render(<App />, ...) 里的 <App />
container, parentComponent, expirationTime, callback) {
  // TODO: If this is a nested container, this won't be the root.
  // 这个 TODO 的意思需要要搞个 while 循环去找 root？
