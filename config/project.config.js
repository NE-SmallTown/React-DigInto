...
const ip = require('ip');
const fileLinkInReactRepo = require('./fileLinkInReactRepo');
...
// get all tested files name to use for sidebar link
const allTestedFiles = {
  // [dir]: {
  //    [fileName]: { // the file name in the /tests directory
  //      menuTitle: '' // the title displayed in sidebar menu
  //      fileLinkInReactRepo: '' // every menu item map to one test unit in react repo
  //      fileContent: '' // the file content of the file with the fileName
  //    }
  //  }
};
...
fs.readdirSync(path.resolve(__dirname, '../src/tests')).forEach(dir => {
  const curDir = {};
  allTestedFiles[dir] = curDir;

  const dirPath = path.resolve(__dirname, `../src/tests/${dir}`);
...
config.globals = {
  'allTestedFiles': JSON.stringify(allTestedFiles),
  'process.env'  : {
